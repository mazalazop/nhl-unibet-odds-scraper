...
def main():
    ...
    batch_ts = now_ts()
    batch_dir = Path("artifacts") / "unibet_event_points_batch_runner" / batch_ts
    ensure_dir(batch_dir)

    parser_output_root = Path("artifacts") / "unibet_event_points_parser_v3"
    ...
    for idx, url in enumerate(urls, start=1):
        ...
        proc = subprocess.run(
            ["python3", "scrapers/unibet_event_points_parser_v3.py"],
            env=env,
            capture_output=True,
            text=True
        )
